 ...
 * until reboot (an idle LAN port stops reaching its peer, the WAN port loses
 * PPPoE PADI across a redial). The vendor ssdk driver clears EEE on every port
 * at init for the same reason. The stubs themselves are never called.
 */
