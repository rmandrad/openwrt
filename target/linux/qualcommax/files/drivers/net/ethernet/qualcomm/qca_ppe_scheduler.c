...
static void ppe_l0_scheduler_init(struct qca_ppe_priv *priv)
{
	...
	for (i = 0; i < ARRAY_SIZE(port_l0); i++) {
		 ...
		 * about one dequeue per second until the node is rebuilt or the
		 * box reboots. The low slots carry best-effort unicast and all
		 * flooding, so that pairing would be exercised continuously;
		 * the top slots stay idle unless skb->priority selects them.
		 */
		for (k = 0; k < 2; k++) {
